...
if __name__ == "__main__":
    ...
    plt.plot(preds[best_idx], label='Pred (Trans)', color='green', linestyle='--')
    plt.title(f'Trans Best Case (MAPE: {sample_mapes[best_idx]:.2f}%)')
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.subplot(1, 3, 2)
    plt.plot(trues[random_idx], label='Truth', color='black')
    plt.plot(preds[random_idx], label='Pred (Trans)', color='blue', linestyle='--')
    plt.title(f'Trans Random Case (MAPE: {sample_mapes[random_idx]:.2f}%)')
    plt.legend()
    plt.grid(True, alpha=0.3)

    plt.subplot(1, 3, 3)
    plt.plot(trues[worst_idx], label='Truth', color='black')
    plt.plot(preds[worst_idx], label='Pred (Trans)', color='red', linestyle='--')
    plt.title(f'Trans Worst Case (MAPE: {sample_mapes[worst_idx]:.2f}%)')
    plt.legend()
    plt.grid(True, alpha=0.3)
